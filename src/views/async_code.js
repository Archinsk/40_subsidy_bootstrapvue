...
function CheckForPlugIn_Async() {
  ...
  function VersionCompare_Async(StringVersion, ObjectVersion) {
    ...
    cadesplugin.async_spawn(function* () {
      ...
      var ret =
        (yield ver.MajorVersion) +
        "." +
        (yield ver.MinorVersion) +
        "." +
        (yield ver.BuildVersion);
      document.getElementById("CSPVersionTxt").innerHTML =
        "Версия криптопровайдера: " + ret;
    ...
    try {
      ...
      for (var i = 1; i <= certCnt; i++) {
        ...
        for (var j = 0; j < global_selectbox_container.length; j++) {
          if (
            (yield global_selectbox_container[j].Thumbprint) ===
...
function FillCertInfo_Async(certificate, certBoxId, isFromContainer) {
  ...
  cadesplugin.async_spawn(
    function* (args) {
      ...
      document.getElementById(args[2] + "from").innerHTML =
        "Выдан: <b>" + Adjust.GetCertDate(ValidFromDate) + " UTC<b>";
      ...
      document.getElementById(args[2] + "algorithm").innerHTML =
        "Алгоритм ключа: <b>" + fAlgoName + "<b>";
      if (hasPrivateKey) {
        var oPrivateKey = yield args[0].PrivateKey;
        var sProviderName = yield oPrivateKey.ProviderName;
        document.getElementById(args[2] + "provname").innerHTML =
          "Криптопровайдер: <b>" + sProviderName + "<b>";
        try {
...
function Encrypt_Async() {
  cadesplugin.async_spawn(function* () {
    ...
    try {
      ...
      try {
        var oSymAlgo = yield cadesplugin.CreateObjectAsync(
...
function RetrieveCertificate_Async() {
  cadesplugin.async_spawn(function* (arg) {
    try {
      var PrivateKey = yield cadesplugin.CreateObjectAsync(
    ...
    yield PrivateKey.propset_ProviderName(
    ...
    try {
      var CertificateRequestPkcs10 = yield cadesplugin.CreateObjectAsync(
    ...
    yield CertificateRequestPkcs10.InitializeFromPrivateKey(
      0x1,
      PrivateKey,
      ""
    );
    ...
    try {
      var DistinguishedName = yield cadesplugin.CreateObjectAsync(
    ...
    yield DistinguishedName.Encode(
      'CN="' + CommonName.replace(/"/g, '""') + '"'
    );

    yield CertificateRequestPkcs10.propset_Subject(DistinguishedName);

    var KeyUsageExtension = yield cadesplugin.CreateObjectAsync(
    ...
    yield (yield CertificateRequestPkcs10.X509Extensions).Add(
      KeyUsageExtension
    );

    try {
      var Enroll = yield cadesplugin.CreateObjectAsync(
  ...
  cadesplugin.async_spawn(function* () {
    ...
    if ((yield CurrentPluginVersion.MajorVersion) == parseInt(arr[0])) {
      if ((yield CurrentPluginVersion.MinorVersion) == parseInt(arr[1])) {
        if ((yield CurrentPluginVersion.BuildVersion) == parseInt(arr[2])) {
          ...
          (yield CurrentPluginVersion.BuildVersion) < parseInt(arr[2])
        ) {
          isActualVersion = false;
        }
    ...
    if (!isActualVersion) {
      document
        ...
        .setAttribute("src", "Img/yellow_dot.png");
      document.getElementById("PlugInEnabledTxt").innerHTML =
        "Плагин загружен, но он не поддерживает УЭК.";
    } else {
      document
        .getElementById("PluginEnabledImg")
        .setAttribute("src", "Img/green_dot.png");
      ...
      if (!isUECCSPInstalled) {
        document
          ...
          .setAttribute("src", "Img/yellow_dot.png");
        document.getElementById("PlugInEnabledTxt").innerHTML =
          "Плагин загружен. Не установлен УЭК CSP.";
      }
    }
    document.getElementById("PlugInVersionTxt").innerHTML =
      "Версия плагина: " + (yield CurrentPluginVersion.toString());
    document.getElementById("CSPVersionTxt").innerHTML =
      "Версия УЭК CSP: " + (yield UECCSPVersion.toString());
  }); //cadesplugin.async_spawn
}
...
function getUECCertificate_Async() {
  return new Promise(function (resolve, reject) {
    showWaitMessage(
      "Выполняется загрузка сертификата, это может занять несколько секунд..."
    );
    cadesplugin.async_spawn(
      function* (args) {
        try {
          ...
          if (typeof oCertTemp == "undefined") {
            ...
            document.getElementById("certerror").innerHTML =
              "Сертификат не найден или отсутствует.";
            throw "";
          }
          ...
          if (yield FoundCertInStore_Async(oCertTemp)) {
            ...
            document.getElementById("cert_info1").style.display = "";
            document.getElementById("certerror").innerHTML =
              "Сертификат не найден в хранилище MY.";
            throw "";
          }
          args[0]();
        } catch (e) {
          ...
          if ("The action was cancelled by the user. (0x8010006E)" == message) {
            document.getElementById("cert_info1").style.display = "";
            document.getElementById("certerror").innerHTML =
              "Карта не найдена или отсутствует сертификат на карте.";
          }
          args[1]();
        }
      },
    ...
    cadesplugin.async_spawn(
      function* (args) {
        ...
        try {
          ...
          showErrorMessage(
            "Ошибка: Не удалось создать подпись. Код ошибки: " +
              cadesplugin.getLastError(e)
          );
          args[1]("");
        }
