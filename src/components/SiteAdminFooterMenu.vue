    ...
    <div>
        ...
        <div class="form-row">
            ...
            <div class="form-group col-2">
                <button
                        @click="resetChanges"
                        class="btn btn-outline-primary btn-block">Отмена
                </button>
            </div>
            <div class="form-group col-2">
                <button
                        @click="applyChangesFooterMenu"
                        class="btn btn-primary btn-block">Применить
                </button>
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            addFooterMenuItem() {
                const xhr = new XMLHttpRequest();
                const url = 'http://192.168.18.171:8080/open-core/api/site-data/set-footer';
            ...
            removeFromChangeList(changedItem) {
                this.changedFooterMenuItemsId = this.changedFooterMenuItemsId.filter(value => value !== changedItem.id);
                this.footerMenu.forEach((item) => {
                    if (item.id === changedItem.id) {
                        delete item.changed;
                ...
                console.log('удаление из массива иземенений: ' + this.changedFooterMenuItemsId)
            },
            resetChanges() {
            ...
            applyChangesFooterMenu() {
                ...
                console.log(qwer);
                let changedItems = this.footerMenu.filter((item) => qwer.includes(item.id));
