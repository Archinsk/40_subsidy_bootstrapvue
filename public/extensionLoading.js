...
function setExtensionLoadingInfo() {
  ...
  if (document.getElementById("ExtensionEnabledImg"))
    document
      .getElementById("ExtensionEnabledImg")
      .setAttribute("src", "Img/green_dot.png");
  if (document.getElementById("ExtensionEnabledTxt"))
    document.getElementById("ExtensionEnabledTxt").innerHTML =
...
function extensionLoadedCallback() {
  window.onload = function (e) {
    ...
    console.log(
      '---Выполнение после загрузки окна колбэка "Расширение загружено"'
    );
    setExtensionLoadingInfo();
  };
  console.log("---Точка 1.1");
  console.log(
    '---Вызов "window.cadesplugin_extension_loaded_callback()" из cadesplugin.api'
  );
  setExtensionLoadingInfo();
