function checkExtension(needToCheck = false) {
  ...
  //already loaded
  if (window.cadesplugin && !needToCheck) return;
    ...
    if (isIE()) {
      ...
      if (name.match(/X509Enrollment/i)) {
        try {
          ...
          var objCertEnrollClassFactory =
            document.getElementById("webClassFactory");
          return objCertEnrollClassFactory.CreateObject(name);
        } catch (e) {
          try {
            var objWebClassFactory = document.getElementById(
  ...
  function nmcades_api_onload() {
    if (!isIE() && !isFireFox && !isSafari) {
      ...
      console.log(
        "---Запуск колбэка extensionLoading в случае успешной загрузки расширения для Хрома"
      );
      if (window.cadesplugin_extension_loaded_callback)
        window.cadesplugin_extension_loaded_callback();
    }
    ...
    window.addEventListener(
      ...
      function (event) {
        ...
        if (isFireFox || isSafari) {
          ...
          if (
            !url.match(
              "^moz-extension://[a-zA-Z0-9-]+/nmcades_plugin_api.js$"
            ) &&
            !url.match(
