...
function FillCertInfo_NPAPI(certificate, certBoxId, isFromContainer) {
  ...
  if (Now < ValidFromDate) {
    ...
    document.getElementById(field_prefix + "status").innerHTML =
      "Статус: <b> Действителен<b>";
  }
  if (isFromContainer) {
    document.getElementById(field_prefix + "location").innerHTML =
      "Установлен в хранилище: <b>Нет</b>";
  } else {
    document.getElementById(field_prefix + "location").innerHTML =
      "Установлен в хранилище: <b>Да</b>";
  }
}

function MakeCadesBesSign_NPAPI(
  dataToSign,
  certObject,
  setDisplayData,
  isBase64
...
function MakeCadesEnhanced_NPAPI(
  dataToSign,
  tspService,
  certObject,
  sign_type
  ...
  try {
    ...
    alert(
      "Failed to create CAdESCOM.CadesSignedData: " +
        cadesplugin.getLastError(err)
    );
    return;
  }
...
function MakeXMLSign_NPAPI(dataToSign, certObject, signatureType) {
  ...
  try {
    ...
    alert(
      "Failed to create CAdESCOM.SignedXML: " + cadesplugin.getLastError(err)
    );
    return;
  }
...
function GetSignatureTitleElement() {
  ...
  if (x.length == 0) {
    elementSignatureTitle =
      document.getElementById("SignatureTxtBox").parentNode.previousSibling;
...
function SignCadesBES_NPAPI(certListBoxId, data, setDisplayData) {
  ...
  try {
    var signature = MakeCadesBesSign_NPAPI(
      dataToSign,
      certificate,
      setDisplayData
    );
...
function SignCadesBES_NPAPI_File(certListBoxId) {
  ...
  try {
    ...
    document.getElementsByName("TimeTitle")[0].innerHTML =
...
function SignCadesEnhanced_NPAPI(certListBoxId, sign_type) {
  ...
  var x = GetSignatureTitleElement();
  try {
    var signature = MakeCadesEnhanced_NPAPI(
      dataToSign,
      tspService,
      certificate,
      sign_type
...
function MakeVersionString(oVer) {
  ...
  if (typeof oVer == "string") return oVer;
    ...
    return (
      oVer.MajorVersion + "." + oVer.MinorVersion + "." + oVer.BuildVersion
    );
}
...
function CheckForPlugIn_NPAPI() {
  ...
  function GetCSPVersion_NPAPI() {
    try {
      ...
      alert(
        "Failed to create CAdESCOM.About: " + cadesplugin.getLastError(err)
      );
      return;
    }
    ...
    window.onload = function (e) {
      document
        .getElementById("CspEnabledImg")
        .setAttribute("src", "Img/green_dot.png");
      document.getElementById("CspEnabledTxt").innerHTML =
  ...
  function ShowCSPVersion_NPAPI(CurrentPluginVersion) {
    if (typeof CurrentPluginVersion != "string") {
      document.getElementById("CSPVersionTxt").innerHTML =
        "Версия криптопровайдера: " + GetCSPVersion_NPAPI();
    }
    var sCSPName = GetCSPName_NPAPI();
    if (sCSPName != "") {
      document.getElementById("CSPNameTxt").innerHTML =
        "Криптопровайдер: " + sCSPName;
    }
  }
  function GetLatestVersion_NPAPI(CurrentPluginVersion) {
    ...
    xmlhttp.open(
      "GET",
      "/sites/default/files/products/cades/latest_2_0.txt",
      true
    );
      ...
      if (xmlhttp.readyState == 4) {
        if (xmlhttp.status == 200) {
          ...
          if (isPluginWorked) {
            ...
            if (
              VersionCompare_NPAPI(PluginBaseVersion, CurrentPluginVersion) < 0
            ) {
              document
  ...
  try {
    ...
    document.getElementById("PlugInVersionTxt").innerHTML =
      "Версия плагина: " + MakeVersionString(CurrentPluginVersion);
...
function GetFirstCert_NPAPI() {
  ...
  try {
    ...
    if (certCnt == 0) throw "Certificate not found";
  } catch (ex) {
    oStore.Close();
  ...
  if (certCnt) {
    try {
      ...
      alert(
        "Ошибка при перечислении сертификатов: " + cadesplugin.getLastError(ex)
      );
      return;
    }
  }
  ...
  if (MyStoreExists) {
    ...
    for (var i = 1; i <= certCnt; i++) {
      ...
      try {
        ...
        oOpt.text =
          CertStatusEmoji(cert.ValidToDate > Date.now()) +
          certObj.GetCertString();
      } catch (ex) {
        alert(
...
function CreateCertRequest_NPAPI() {
  try {
    ...
    alert(
      "Failed to create X509Enrollment.CX509PrivateKey: " +
  ...
  try {
    var CertificateRequestPkcs10 = cadesplugin.CreateObject(
      "X509Enrollment.CX509CertificateRequestPkcs10"
    );
  } catch (e) {
    alert(
      "Failed to create X509Enrollment.CX509CertificateRequestPkcs10: " +
        cadesplugin.getLastError(e)
    );
    return;
  }
  ...
  try {
    var DistinguishedName = cadesplugin.CreateObject(
      "X509Enrollment.CX500DistinguishedName"
    );
  } catch (e) {
    alert(
      "Failed to create X509Enrollment.CX500DistinguishedName: " +
        cadesplugin.getLastError(e)
    );
    return;
  }
  ...
  var KeyUsageExtension = cadesplugin.CreateObject(
  ...
  try {
    ...
    alert(
      "Failed to create X509Enrollment.CX509Enrollment: " +
        cadesplugin.getLastError(e)
    );
    return;
  }
  ...
  try {
    ...
    alert(
      "Failed to generate KeyPair or reguest: " + cadesplugin.getLastError(e)
    );
    return;
  }
...
function RetrieveCertificate_NPAPI() {
  ...
  var params =
    "CertRequest=" +
    encodeURIComponent(cert_req) +
    "&Mode=" +
    encodeURIComponent("newreq") +
    "&TargetStoreFlags=" +
  ...
  xmlhttp.open(
    "POST",
    "https://testca.cryptopro.ru/certsrv/certfnsh.asp",
    true
  );
  ...
  xmlhttp.onreadystatechange = function () {
    if (xmlhttp.readyState == 4) {
      if (xmlhttp.status == 200) {
        ...
        try {
          var Enroll = cadesplugin.CreateObject(
...
function Encrypt_NPAPI() {
  ...
  var dataToEncr1 = Base64.encode(
    document.getElementById("DataToEncrTxtBox1").value
  );
  var dataToEncr2 = Base64.encode(
    document.getElementById("DataToEncrTxtBox2").value
  );
  ...
  try {
    ...
    document.getElementById("DataEncryptedDiversData1").value =
      oSesKey1DiversData;
    ...
    document.getElementById("DataEncryptedDiversData2").value =
      oSesKey2DiversData;
...
function Decrypt_NPAPI(certListBoxId) {
  ...
  var field;
  if (certListBoxId == "CertListBox1") field = "DataEncryptedKey1";
  else field = "DataEncryptedKey2";
  ...
  try {
    ...
    var oSesKey1DiversData = document.getElementById(
      "DataEncryptedDiversData1"
    ).value;
    ...
    document.getElementById("DataDecryptedBox1").value =
      Base64.decode(EncryptedData1);
    var oSesKey2DiversData = document.getElementById(
      "DataEncryptedDiversData2"
    ).value;
...
function isIE() {
  var retVal =
    "Microsoft Internet Explorer" == navigator.appName || // IE < 11
...
function ShowEdgeNotSupported() {
  document
    ...
    .setAttribute("src", "Img/red_dot.png");
  document.getElementById("PlugInEnabledTxt").innerHTML =
    "К сожалению, браузер Edge не поддерживается!";
}
...
var MakePayment = function (sum, date, to) {
  return (
    "<!PINPADFILE UTF8><N>Платежное поручение<V>500" +
    "<N>Сумма<V>" +
    sum +
    "<N>Дата<V>" +
    date +
...
function ShowPinPadelogin() {
  ...
  var text =
    "<!PINPADFILE UTF8><N>Авторизация<V><N>Подтвердите авторизацию на сайте<V>" +
    "cryptopro.ru" +
    "<N>Вход будет произведен с логином<V>" +
    loginvalue;
